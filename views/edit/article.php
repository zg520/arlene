...
<div class="column fullSized">
	<div id="statusSubmit">
		<span>Status: <span id="articleStatus" title="<?php echo $this -> viewBag['article'] -> status ?>"><?php echo str_replace("_", " ", $this -> viewBag['article'] -> status) ?> </span></span>
		<button id="editArticle" class="" style="border-bottom: solid #FF3333">Edit <span class="ui-icon ui-icon-pencil"></span></button>
	</div>
	<article>
		<header>
			<h1 id="articleTitle"><?php echo $this -> viewBag['article'] -> title; ?></h1>
			<h2 class="hidden" id="articleId"><?php echo $this -> viewBag['article'] -> id; ?></h2>
			<img id="articleImage" src="<?php echo $this -> viewBag['article'] -> coverUrl?>" alt="Cover Image">
	</header>
 		<p id="articleText"><?php echo $this -> viewBag['article'] -> body; ?></p>
  <footer style="text-align: right">
  	<p>Created: <time pubdate datetime="<?php echo $this -> viewBag['article'] -> publishDate; ?>"><?php echo $this -> viewBag['article'] -> publishDate; ?></time></p>
  	<p>Likes: <?php echo $this -> viewBag['article'] -> likes; ?> vs <?php echo $this -> viewBag['article'] -> dislikes; ?> Dislikes</p>
  	<small>Writers: <?php
 ...
 ?></small></footer>
  ...
  <section>
    ...
    <?php endif; ?>
    <?php foreach($this -> viewBag['article'] -> editorComments as $comment): ?>
    <article>
      <header>
