...
abstract class ContentManager extends DataManager {
	 ...
	 * @access public
	 *
	 * @param string $id The content piece id.
	 *
	...
	/**
	 * Changes the status of a content piece.
	 *
	 * @access public
	 *
	...
	public function changeStatus($id, $editorId, $newStatus) {
		$this -> upsert("DELETE FROM `publishmetadata` WHERE `article_id` = :article_id", array('article_id' => $id));
	...
	/**
	 * Adds an editor comment to a content piece.
	 *
	 * @access public
	 *
	 * @param string $id The id of the article to add a comment to.
	...
	/**
	 * Adds a user comment to a content piece.
	 *
	 * @access public
	 *
	 * @param string $id The id of the article to add a comment to.
	 ...
	 *
	 * @return Article The base article from the database or false.
	 */
	...
	protected function getWriterArticlesIds($userId, $status) {
		$result = $this -> query("SELECT `id` FROM `articlewriters` INNER JOIN `articles` ON `articles`.`id` = `articlewriters`.`article_id` WHERE `user_id` = ? AND `status` = ?", array($userId, $status));
		return $result;
	}

	/**
	 * Fetches all content from a list with the most popular content in the range specified.
	...
	/**
	 * Fetches all content from a list with recommended content in the range specified.
	 * The default range is 0 - 5 content pieces.
	 *
	 * @access protected
	 *
	...
	/**
	 * Fetches a number of content Ids by specified status, type.
	 * Default range for the operation is 0-5 content pieces.
	 *
	 * @access protected
	 *
	 * @param string $status The status of the content piece.
	 * @param string $type The type of the content piece.
	 * @param int $top How many to take.
	 * @param int $skip How many to skip.
	 * @return array Article ids.
	 */
	...
	/**
	 * Gets the comments of the content.
	 *
	 * @access protected
	 *
	 * @param string $id The content id.
	...
	/**
	 * Gets the number of likes or dislikes for a particular content piece.
	 *
	 * @access protected
	 *
	...
	/**
	 * Adds a user vote to a content piece.
	 *
	 * @access protected
	 *
	 * @param string $id The id of the article.
